// ==========================================
        ...
        const ROOT_TAXA = [
            { id: 40151, name: "哺乳纲", desc: "Mammalia", rank: "class" },
            { id: 3, name: "鸟纲", desc: "Aves", rank: "class" },
            { id: 26036, name: "爬行纲", desc: "Reptilia", rank: "class" },
            { id: 20978, name: "两栖纲", desc: "Amphibia", rank: "class" },
            { id: 47178, name: "辐鳍鱼纲", desc: "Actinopterygii", rank: "class" },
            { id: 47273, name: "软骨鱼纲", desc: "Chondrichthyes", rank: "class" },
            { id: 47158, name: "昆虫纲", desc: "Insecta", rank: "class" },
        ...
        const UI = {
            ...
            tabInat: document.getElementById('tab-inat')
        };
        ...
        function createTreeNode(taxon, isRoot = false) {
            ...
            childrenContainer.className = 'hidden'; nodeDiv.appendChild(childrenContainer);
            return nodeDiv;
        }
        ...
        function getApiUrl() {
            ...
            if (State.placeId) {
                ...
                url += `taxa?taxon_id=${State.baseTaxonId}&order_by=observations_count&order=desc`;
            }
        ...
        // ==========================================
        // 双源阅读器：Wikipedia vs iNaturalist
        // ==========================================
        async function openFullWikiModal() {
            ...
            ModalCache = { wikiHtml: null, inatHtml: null, activeTab: "wiki" };
            switchModalSource('wiki');
        }

        async function switchModalSource(source) {
            ModalCache.activeTab = source;

            if (source === 'wiki') {
                UI.tabWiki.className = "px-3 py-1.5 rounded bg-teal-600 text-white font-bold transition";
                UI.tabInat.className = "px-3 py-1.5 rounded text-teal-300 hover:text-white transition cursor-pointer";
            } else {
                UI.tabInat.className = "px-3 py-1.5 rounded bg-teal-600 text-white font-bold transition";
                UI.tabWiki.className = "px-3 py-1.5 rounded text-teal-300 hover:text-white transition cursor-pointer";
            }

            if (source === 'wiki' && ModalCache.wikiHtml) { UI.modalContent.innerHTML = ModalCache.wikiHtml; return; }
            if (source === 'inat' && ModalCache.inatHtml) { UI.modalContent.innerHTML = ModalCache.inatHtml; return; }
            ...
            UI.modalContent.innerHTML = `
                <div class="absolute inset-0 flex flex-col items-center justify-center text-gray-500 bg-white">
                    ...
                    <p class="font-bold text-lg">正在从 ${source === 'wiki' ? 'Wikipedia' : 'iNaturalist'} 提取档案...</p>
                </div>`;

            try {
                if (source === 'wiki') {
                    let fullHtml = await fetchWikiData(State.currentWikiTitle, "html");
                    if (!fullHtml && State.currentTarget.name) fullHtml = await fetchWikiData(State.currentTarget.name, "html");

                    if (fullHtml) {
                        ModalCache.wikiHtml = sanitizeWikiHtml(fullHtml);
                    } else {
                        ModalCache.wikiHtml = `<div class="text-center text-gray-500 mt-20"><i class="fa-solid fa-ghost text-5xl mb-3"></i><br>中文维基百科暂无该物种文献，请尝试切换到 iNaturalist 或访问原网页。</div>`;
                    }
                    if (ModalCache.activeTab === 'wiki') UI.modalContent.innerHTML = ModalCache.wikiHtml;
                }
                else if (source === 'inat') {
                    const inatRes = await fetch(`https://api.inaturalist.org/v1/taxa/${State.currentTarget.id}?locale=zh-CN`);
                    const inatData = await inatRes.json();
                    if (inatData.results[0] && inatData.results[0].wikipedia_summary) {
                        ModalCache.inatHtml = `<div class="bg-blue-50 p-4 rounded-xl mb-6 text-blue-800 text-sm border border-blue-200 shadow-sm"><i class="fa-solid fa-circle-info"></i> 以下内容由 iNaturalist 核心库提供：</div>` + sanitizeWikiHtml(inatData.results[0].wikipedia_summary);
                    } else {
                        ModalCache.inatHtml = `<div class="text-center text-gray-500 mt-20"><i class="fa-solid fa-ghost text-5xl mb-3"></i><br>iNaturalist 暂无该物种的中文概述。</div>`;
                    }
                    if (ModalCache.activeTab === 'inat') UI.modalContent.innerHTML = ModalCache.inatHtml;
                }
            } catch (error) {
